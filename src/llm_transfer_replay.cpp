...
void usage(std::ostream& stream) {
    stream << "Usage: llm_transfer_replay --trace transfer-events.csv "
              ...
              "--output ocean-service-events.csv [--metadata-output replay-metadata.json] "
...
Options parseOptions(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--trace") options.trace = value;
        else if (argument == "--hardware-profile") options.hardware_profile = value;
        else if (argument == "--output") options.output = value;
        else if (argument == "--metadata-output") options.metadata_output = value;
    ...
    if (std::filesystem::path(options.trace).extension() != ".csv") {
        throw std::runtime_error("standalone OCEAN frontend accepts versioned CSV; use cxl-llm to convert Parquet");
    }
    if (std::filesystem::path(options.output).extension() != ".csv") {
        throw std::runtime_error("standalone OCEAN frontend writes CSV; use cxl-llm to convert it to Parquet");
    }
    if (options.metadata_output.empty()) options.metadata_output = options.output + ".metadata.json";
    return options;
}
...
int main(int argc, char** argv) {
    try {
        ...
        const auto events = model.replay(requests, mode);
        cxlmemsim::llm::writeServiceEventsCsv(options.output, events);
        cxlmemsim::llm::writeReplayMetadataJson(options.metadata_output, profile, events, mode);
